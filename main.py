    ...
    def _loop(self):
        while self.running:
            try:
                ...
                moderation_system.cleanup_expired_mutes()
...
# Обновите функцию проверки сообщений:
def process_webhook_user_message(msg):
    ...
    try:
        ...
        logger.info(f"🔍 Проверка сообщения от {user_id} в чате {peer_id}")
        ...
        if moderation_system.should_delete_message(vk, msg, get_user_role):
            logger.info(f"🔇 УДАЛЯЕМ сообщение от {user_id}")
