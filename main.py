...
SUPER_ADMIN_ID = config['super-admin-id']
...
async def private_message_handler(message: Message):
    """
    Обработчик личных сообщений боту
    """
    text = message.text.lower() if message.text else ""
    
    if text in ['привет', 'hello', 'hi', 'start', 'старт', 'help', 'помощь']:
        await message.answer(
            "👋 Привет! Я Самый Бот менеджер проекта Arion RP.\n\n"
            "🌐 Форум проекта: https://forum.arion-rp.ru\n\n"
            "⬇️ Скачать игру: https://arion-rp.ru/download\n\n"
        )
    else:
        await message.answer(
            "👋 Привет! Я Самый Бот менеджер проекта Arion RP.\n\n"
            "🌐 Форум проекта: https://forum.arion-rp.ru\n\n"
            "⬇️ Скачать игру: https://arion-rp.ru/download\n\n"
            "Напишите 'help' для получения полной информации о командах."
        )
...
async def get_role(user_id=int, chat_id=int):
    # Проверяем, является ли пользователь супер-админом из конфига
    if user_id == SUPER_ADMIN_ID:
        return 6  # Уровень менеджера бота (самый высокий)
...
async def nlist(chat_id=int, page=int):
    ...
    nicks = []
    gi = 0
    max_nicks = config['nicks-max']  # Используем config вместо open_file
    for i in fetch:
        gi = gi + 1
...
async def nonick(chat_id=int, page=int):
    ...
    gi = 0
    nonick = []
    max_nonick = config['nonick-max']  # Используем config вместо open_file
    ...
    if command_identifier in bot_identifiers:
        ...
        if command in ['start', 'старт', 'активировать']:
            ...
            try:
                ...
                # Разрешаем активацию как владельцу беседы, так и супер-админу
                if owner != user_id and user_id != SUPER_ADMIN_ID:
                    await message.reply("Включить бота может только создатель беседы или супер-админ бота!", disable_mentions=1)
                    return True
                
                await new_chat(chat_id, peer_id, owner)  # Используем реального владельца беседы
                await message.reply(
                    f"✅ Бот успешно запущен!\n\n"
                    f"📖 Для просмотра команд напишите:\n"
                    f"/help - основные команды\n"
                    f"/alt - альтернативные названия\n\n"
                    f"⚙️ Рекомендуемая начальная настройка:\n"
                    f"1. Назначьте модераторов: /addmoder\n"
                    f"2. Настройте приветствие: /welcome\n"
                    f"3. Установите режим тишины при необходимости: /quiet",
                    disable_mentions=1
                )
            except Exception as ex:
                await message.reply(f"❌ Ошибка активации! Убедитесь, что боту выданы права администратора (звезда) в беседе. Ошибка: {ex}", disable_mentions=1)
                return True
