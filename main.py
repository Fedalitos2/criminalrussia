    ...
    if invited_user == -224437676:
        ...
        if welcome:
            ...
            await message.answer(welcome)
                ...
                # После проверки бана добавьте:
        # Проверка черного списка
        for list_type in ['admins', 'leaders', 'users']:
            blacklist_info = db.check_blacklist(invited_user, list_type)
            if blacklist_info:
                try:
                ...
                await message.answer(
                    f"⚠️ @id{invited_user} находится в черном списке ({list_type})\n"
                    f"Причина: {blacklist_info['reason']}\n"
                    f"До: {blacklist_info['until_date']}",
                    disable_mentions=1
                )
                return True
...
async def handlers(message: GroupTypes.MessageEvent):
    ...
    # В обработчике команд добавьте:

    if command in ['blacklist', 'bl', 'чс']:
        if not is_super_admin(user_id):
            await message.reply("Недостаточно прав! Только главный администратор может управлять черным списком.", disable_mentions=1)
            return True
        
        if len(arguments) < 2:
            # Показать справку по черному списку
            help_text = """
            Команды черного списка:
            /blacklist add <тип> <ссылка> <дней> <причина> - Добавить в ЧС
            /blacklist remove <тип> <ссылка> - Удалить из ЧС
            /blacklist check <тип> <ссылка> - Проверить ЧС
            /blacklist list <тип> - Список ЧС
            
            Типы ЧС:
            - admins (админы бота)
            - leaders (владельцы бесед)
            - users (обычные пользователи)
            
            Пример:
            /blacklist add users @id1234567 30 Спам
            /blacklist check users @id1234567
            """
            await message.reply(help_text, disable_mentions=1)
            return True
        
        subcommand = arguments_lower[1]
        
        if subcommand == 'add':
            if len(arguments) < 6:
                await message.reply("Недостаточно аргументов. Формат: /blacklist add <тип> <ссылка> <дней> <причина>", disable_mentions=1)
                return True
            
            list_type = arguments_lower[2]
            target_user = await getID(arguments[3])
            days = int(arguments[4])
            reason = ' '.join(arguments[5:])
            
            if list_type not in ['admins', 'leaders', 'users']:
                await message.reply("Неверный тип ЧС. Доступные: admins, leaders, users", disable_mentions=1)
                return True
            
            if not target_user:
                await message.reply("Не удалось распознать пользователя", disable_mentions=1)
                return True
            
            db.add_to_blacklist(target_user, reason, days, list_type)
            await message.reply(f"✅ Пользователь добавлен в ЧС ({list_type}) на {days} дней. Причина: {reason}", disable_mentions=1)
        
        elif subcommand == 'remove':
            if len(arguments) < 4:
                await message.reply("Недостаточно аргументов. Формат: /blacklist remove <тип> <ссылка>", disable_mentions=1)
                return True
            ...
            db.remove_from_blacklist(target_user, list_type)
            await message.reply(f"✅ Пользователь удален из ЧС ({list_type})", disable_mentions=1)
        
        elif subcommand == 'check':
            if len(arguments) < 4:
                await message.reply("Недостаточно аргументов. Формат: /blacklist check <тип> <ссылка>", disable_mentions=1)
                return True
            ...
            blacklist_info = db.check_blacklist(target_user, list_type)
            if blacklist_info:
                info_text = f"""
                🚫 Пользователь в черном списке ({list_type}):
                Причина: {blacklist_info['reason']}
                До: {blacklist_info['until_date']}
                Добавлен: {blacklist_info['added_at']}
                """
                await message.reply(info_text, disable_mentions=1)
            else:
                await message.reply(f"✅ Пользователь не в черном списке ({list_type})", disable_mentions=1)
        
        elif subcommand == 'list':
            if len(arguments) < 3:
                await message.reply("Недостаточно аргументов. Формат: /blacklist list <тип>", disable_mentions=1)
                return True
            ...
            blacklist = db.get_blacklist_all(list_type)
            if not blacklist:
                await message.reply(f"📭 Черный список ({list_type}) пуст", disable_mentions=1)
                return True
            
            response = f"📋 Черный список ({list_type}):\n\n"
            for i, item in enumerate(blacklist[:20], 1):
                response += f"{i}. @id{item[0]} - {item[1]}\n"
                response += f"   До: {item[2]}\n\n"
