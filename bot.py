...
def process_message(message):
    ...
    # ===================== ОСНОВНЫЕ КОМАНДЫ =====================
        ...
        keyboard = {
            "inline": True,
            "buttons": [
                [{"action": {"type": "callback", "label": "Список команд", "payload": '{"cmd":"bhelp"}'}, "color": "negative"}],
                [{"action": {"type": "callback", "label": "Публичная оферта", "payload": '{"cmd":"boffer"}'}, "color": "positive"}],
            ]
        }
        
        start_message = (f"Привет! 🤙\n"
                        f"Ты попал в **{BOT_NAME}** — помощника для CRMP/SAMP проектов!\n"
                        f"👮 Бот помогает управлять чёрными списками игроков.\n"
                        f"🍱 Чтобы начать:\n"
                        f"Попроси главного администратора добавить тебя в список админов\n"
                        f"🕳 Или создай проект и обратись к основателю для добавления в вайт-лист\n"
                        f"☢ Нажми кнопку ниже, чтобы увидеть список команд.\n"
                        f"Полная инструкция: Напишите /help\n"
        ...
        log_action(user_id, f"Вывел приветствие")
        
    elif text.startswith("/help"):
        help_message = (
            f"📋 **{BOT_NAME} - Список команд**\n\n"
            f"🔰 **Основные команды:**\n"
            f"• /start - Приветственное сообщение\n"
            f"• /help - Этот список\n"
            f"• /offer - Публичная оферта\n"
            f"• /ping - Проверка работы бота\n"
            f"• /ip - IP сервера бота\n\n"
            f"🔰 **Управление ЧС:**\n"
            f"• /ban [id] [дни] [причина] - Забанить\n"
            f"• /aban [id] [причина] - Перманентный бан\n"
            f"• /unban [id] - Разбанить\n"
            f"• /checkban [id] - Проверить бан\n"
            f"• /banlist - Список заблокированных\n"
            f"• /sysban [id] [причина] - Системный бан\n"
            f"• /offsysban [id] - Снять системный бан\n"
            f"• /syslist - Список системных банов\n\n"
            f"🔰 **Администрация:**\n"
            f"• /admins - Список админов 1-3 уровня\n"
            f"• /staff - Список сотрудников 4-7 уровня\n"
            f"• /setadmin [id] [уровень] - Назначить админа\n"
            f"• /rr [id] - Снять права\n\n"
            f"🔰 **Модерация чата:**\n"
            f"• /kick [id] [причина] - Кикнуть\n"
            f"• /warn [id] [причина] - Выдать варн\n"
            f"• /unwarn [id] - Снять варны\n"
            f"• /warnlist - Список варнов\n"
            f"• /mute [id] [минуты] [причина] - Мут\n"
            f"• /add [id] - Добавить в чат\n"
            f"• /sync - Синхронизировать чат\n\n"
            f"🔰 **Связь:**\n"
            f"• /report [текст] - Отправить вопрос\n"
            f"• /question [текст] - Вопрос админам\n"
            f"• /answer [id] [текст] - Ответить\n"
            f"• /reps - Просмотр репортов (админы 4+)\n"
            f"• /delrep [id/all] - Удалить репорт\n\n"
            f"🔰 **Системные:**\n"
            f"• /logs - Просмотр логов\n"
            f"• /clearlog - Очистить логи\n"
            f"• /panel - Панель управления\n"
            f"• /sysinfo [id] - Информация о пользователе\n"
            f"• /editcmd [команда] [уровень] - Изменить уровень команды\n\n"
            f"👑 **Владелец:** [id709914900|Основатель]\n"
            f"📞 **Поддержка:** Напишите /report\n"
        )
        
        send_message(peer_id, help_message)
        log_action(user_id, f"Вывел /help")
