...
# Глобальные переменные для режима тишины
silence_mode = {}
...
class Scheduler:
    ...
    def _loop(self):
        """Основной цикл планировщика - только для ЧС"""
        while self.running:
            try:
                # ТОЛЬКО очистка истекших записей ЧС
...
def check_user_mute(user_id, peer_id):
    ...
    # Используем mute_system вместо активных мутов
...
def process_dm_message(user_id, text, msg):
    """Обрабатывает личные сообщения (аналог логики из main.py)"""
    current_state = user_states.get(user_id, {})
    
    # Обработка пошаговых действий (упрощенная версия)
    if current_state.get('action'):
        # Здесь должна быть логика обработки состояний
        # Для краткости оставлю заглушку
...
# ========== ФУНКЦИИ ДЛЯ CHAT_COMMANDS.PY ==========

def app_mute_user(peer_id, target_id, moderator_id, duration_minutes, reason, reply_to=None):
    """Выдает мут пользователю (версия для app.py)"""
    ...
    if result:
        ...
        moderator_info = get_user_info(moderator_id)
        ...
        send_chat_message(peer_id, f"❌ Ошибка: {message}", reply_to=reply_to)
