...
def mute_user(vk, peer_id, target_id, moderator_id, duration_minutes, reason, reply_to=None):
    ...
    result, message = moderation_system.mute_user(
        target_id, peer_id, duration_minutes, moderator_id, reason
    )
    ...
    if result:
        ...
        print(f"✅ Мут установлен для {target_id}")
    else:
        send_chat_message(vk, peer_id, f"❌ Ошибка: {message}", reply_to=reply_to)

# Обновите функцию unmute_user:
def unmute_user(vk, peer_id, target_id, moderator_id, reply_to=None):
    """Снимает мут"""
    print(f"🔧 Снимаем мут с пользователя {target_id}")
    
    if moderation_system.unmute_user(target_id, peer_id):
        target_info = get_user_info(vk, target_id)
        moderator_info = get_user_info(vk, moderator_id)
...
def handle_new_chat_commands(vk, msg, user_id, text, peer_id):
    ...
    if text.lower() == 'кто':
        ...
        # Команда /muteinfo - показывает информацию о муте
    if text.lower().startswith('/muteinfo'):
        show_mute_info(vk, msg, user_id, text, peer_id)
        return
    
    # Команда /active_mutes - показывает активные муты
    if text.lower() == '/active_mutes':
        show_active_mutes(vk, msg, user_id, peer_id)
        return
...
# На это:
    ...
    if text.lower() == '/silence off':
        if has_permission(user_id, 3):
            moderation_system.set_silence_mode(peer_id, False)
            send_chat_message(vk, peer_id, "🔊 РЕЖИМ ТИШИНЫ ВЫКЛЮЧЕН", reply_to=msg['id'])
        else:
            send_chat_message(vk, peer_id, "❌ У вас нет прав для управления режимом тишины", reply_to=msg['id'])
        return
    
    # Команда /active_mutes
    if text.lower() == '/active_mutes':
        if has_permission(user_id, 2):
            show_active_mutes_command(vk, peer_id)
        else:
            send_chat_message(vk, peer_id, "❌ У вас нет прав для просмотра мутов", reply_to=msg['id'])
        return
...
• /mute @user [время] [причина] - выдать мут пользователю
  Примеры:
  /mute @id123456 30 Спам
  /mute @id123456 60 Оскорбления
  /mute @id123456 1440 Массовый спам (1 день)
  Доступные интервалы: 5, 15, 30, 60, 180, 1440 минут

• /unmute @user - снять мут с пользователя

• /muteinfo @user - информация о муте пользователя

• /active_mutes - показать активные муты (только админы)
...
💡 ПРИМЕЧАНИЯ:
• Время мута указывается в минутах
• Минимальный мут: 5 минут
• Максимальный мут: 10080 минут (7 дней)
...
def show_mute_info(vk, msg, user_id, text, peer_id):
    """Показывает информацию о муте пользователя"""
    if not has_permission(user_id, 2):
        send_chat_message(vk, peer_id, "❌ У вас нет прав для просмотра информации о мутах", reply_to=msg['id'])
        return
    
    parts = text.split()
    if len(parts) < 2:
        send_chat_message(vk, peer_id, "❌ Использование: /muteinfo @user", reply_to=msg['id'])
        return
    ...
    if not target_id:
        send_chat_message(vk, peer_id, "❌ Не удалось определить пользователя", reply_to=msg['id'])
        return
    
    mute_data = mute_system.get_mute_info(target_id, peer_id)
    ...
    if mute_data:
        ...
        message = f"🔇 ИНФОРМАЦИЯ О МУТЕ\n"
        message += f"👤 Пользователь: {target_info}\n"
        message += f"⏰ Осталось: {minutes_left} минут\n"
        message += f"📅 До: {mute_data['until'].strftime('%d.%m.%Y %H:%M:%S')}\n"
        message += f"👮 Выдал: {moderator_info}\n"
        message += f"📝 Причина: {mute_data['reason'] or 'Не указана'}"
        
        send_chat_message(vk, peer_id, message, reply_to=msg['id'])
    else:
        target_info = get_user_info(vk, target_id)
        send_chat_message(vk, peer_id, f"✅ Пользователь {target_info} не в муте", reply_to=msg['id'])

def show_active_mutes(vk, msg, user_id, peer_id):
    """Показывает активные муты в этом чате"""
    if not has_permission(user_id, 3):  # Только админы
        send_chat_message(vk, peer_id, "❌ У вас нет прав для просмотра активных мутов", reply_to=msg['id'])
        return
    
    active_mutes = mute_system.get_all_active_mutes()
    chat_mutes = [m for m in active_mutes if m['peer_id'] == peer_id]
    
    if not chat_mutes:
        send_chat_message(vk, peer_id, "📭 В этом чате нет активных мутов", reply_to=msg['id'])
        return
    
    message = "🔇 АКТИВНЫЕ МУТЫ В ЧАТЕ:\n\n"
    for i, mute in enumerate(chat_mutes, 1):
        user_info = get_user_info(vk, mute['user_id'])
        moderator_info = get_user_info(vk, mute['moderator'])
        
        message += f"{i}. {user_info}\n"
        message += f"   ⏰ Осталось: {mute['time_left']} мин\n"
        message += f"   👮 Модератор: {moderator_info}\n"
        message += f"   📝 Причина: {mute['reason'] or 'Не указана'}\n\n"
    
    send_chat_message(vk, peer_id, message, reply_to=msg['id'])
    
    # chat_commands.py - добавьте функцию
def show_active_mutes_command(vk, peer_id):
    """Показывает активные муты в чате"""
    active_mutes = moderation_system.get_active_mutes(peer_id)
    
    if not active_mutes:
        send_chat_message(vk, peer_id, "📭 В этом чате нет активных мутов")
        return
    ...
    for i, mute in enumerate(active_mutes, 1):
        user_info = get_user_info(vk, mute['user_id'])
        moderator_info = get_user_info(vk, mute['moderator'])
        time_left = mute['until'] - datetime.now()
        minutes_left = max(0, int(time_left.total_seconds() / 60))
