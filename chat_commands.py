...
def mute_user(vk, peer_id, target_id, moderator_id, duration_minutes, reason, reply_to=None):
    ...
    result, message = mute_system.mute_user(
...
def unmute_user(vk, peer_id, target_id, moderator_id, reply_to=None):
    """Снимает мут"""
    print(f"🔧 Снимаем мут с пользователя {target_id} в чате {peer_id}")
    
    if mute_system.unmute_user(target_id, peer_id):
        target_info = get_user_info(vk, target_id)
        moderator_info = get_user_info(vk, moderator_id)
        ...
        send_chat_message(vk, peer_id, "❌ Пользователь не в муте в этом чате", reply_to=reply_to)
