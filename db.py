    ...
    with sqlite3.connect("database.db") as conn:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS mutes (
                ...
                reason TEXT,
                date TEXT,
                muted_in_chat_id INTEGER,
                PRIMARY KEY(user_id, muted_in_chat_id)
                    )
        ''')
        cursor.execute('''
        ...
        ''')
                ...
                # В функции init_db() после создания других таблиц добавить:
        cursor.execute('''
        ...
        owner_commands = [
            ("/setadmin", 7),
            ("/pin", 2),           # Только админы 2+ уровня
            ("/unpin", 2),         # Только админы 2+ уровня
            ("/snick", 2),         # Только админы 2+ уровня
            ("/rnick", 2),         # Только админы 2+ уровня
            ("/nlist", 0),         # Доступно всем (уровень 0)
            ("/gnick", 1),         # Доступно всем (уровень 0)
            ("/rr", 7),
            ("/editcmd", 7),
            ("/sysban", 1),
            ("/offsysban", 1),
...
DEFAULT_COMMAND_LEVELS = {
    # В db.py в DEFAULT_COMMAND_LEVELS добавить:
    "/pin": 2,
    "/unpin": 2,
    "/snick": 2,
    "/rnick": 2,
...
def add_mute(user_id: int, muted_by_id: int, end_time: datetime, reason: str, chat_id: int):
    """Добавить мут"""
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    ...
    cursor.execute('''
        ...
        (user_id, muted_by_id, mute_end_time, reason, date, muted_in_chat_id) 
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (user_id, muted_by_id, end_time_str, reason, date, chat_id))
    conn.commit()
    conn.close()
...
def remove_mute(user_id: int, chat_id: int):
    ...
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("DELETE FROM mutes WHERE user_id = ? AND muted_in_chat_id = ?", (user_id, chat_id))
    conn.commit()
    conn.close()

def get_mute(user_id: int, chat_id: int):
    """Получить информацию о муте"""
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM mutes WHERE user_id = ? AND muted_in_chat_id = ?",
        (user_id, chat_id)
    )
    row = cursor.fetchone()
    conn.close()
...
def get_all_active_mutes():
    """Получить все активные муты"""
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    ...
    for row in rows:
        end_time = datetime.fromisoformat(row[2])  # mute_end_time
        if now < end_time:
            active_mutes.append({
                "user_id": row[0],
                "end_time": row[2],
            ...
            # Удаляем истёкший мут
            remove_mute(row[0], row[5])  # user_id, muted_in_chat_id
