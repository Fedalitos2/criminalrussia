...
def get_mute(user_id):
    ...
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, end_time, reason, admin_id FROM mutes WHERE user_id = ?", (user_id,))
    row = cursor.fetchone()
    conn.close()
...
def get_all_active_mutes():
    ...
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, end_time, reason, admin_id FROM mutes")
